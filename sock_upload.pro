...
;               28 January 2018, Zarro (ADNET) - added "upload" subdirectory
...
if is_blank(err) then begin
 ...
 odir=target+'/'+session
 file_mkdir,odir
 aname=local_name(odir+'/'+file_basename(fname))
 lname='/'+session+'/'+file_basename(fname)
...
if is_blank(err) then begin
 ...
 printf,lun,'Last-Modified: '+file_time(aname,/http)+cr
