...
;               28 January 2018, Zarro (ADNET) - fixed bug with closing port
;
...
if status then begin
 ...
 sock_recvar,clientlun,name,value,err=err,verbose=verbose,format=format,session=session,$
  disconnected=disconnected,nowait=nowait,main_level=main_level,out_dir=out_dir
...
;-- client sent a file
   ...
   if format eq 'file' then begin
    ...
    if is_string(out_dir) then begin
	 if write_dir(out_dir,err=err) then temp_dir=out_dir else begin
      mprint,err
	  mprint,'Using default output directory.'
	 endelse 
	endif
    file_mkdir,temp_dir
	bsize=n_elements(value)
	mprint,'Writing '+trim(bsize)+' bytes of '+name+' to '+temp_dir
    write_stream,name,value,out_dir=temp_dir
   endif
...
if ~since_version('8.5') then begin
 mprint,'Needs IDL version 8.5 or better.'
 return
endif
...
if is_struct(userdata) then begin
 ...
 userdata={listenerlun:listenerlun,clientlun:clientlun[0],port:long(port),bridge:obj_new(),$
           max_connections:max_connections,server:server,http_mode:http_mode,$
           callback:server_callback}
endelse

if is_socket(listenerlun) then begin
 mprint,server+' listening on port '+trim(port)
 return
endif
