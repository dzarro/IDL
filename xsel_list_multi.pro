...
;   3-Mar-2022, Zarro (ADNET) - added /NO_COUNTER
;   13-Jun-2026, Zarro (Retired) - added /MODAL and XREALIZE to control blocking and screen placement
;-
;===========================================================================
...
pro xsel_list_multi_listwidget, w_baserow=w_baserow, items=items, label=label, initial=initial, uvalue=uvalue,$
                                w_list=w_list,no_counter=no_counter,multiple=multiple
...
cancel=0
do_multi=1b
if is_number(multiple) then if multiple eq 0 then do_multi=0b
...
if do_multi then instruct='(Use shift and control keys to select multiple items.)' else instruct=''
tmp = widget_label (w_basemain, value=instruct)

w_baserow = widget_base(w_basemain, /row, space=10)

xsel_list_multi_listwidget, w_baserow=w_baserow, items=items, $
  label=label, initial=initial, uvalue='select',w_list=w_list,no_counter=no_counter,multiple=do_multi
...
if keyword_set(all) and do_multi then tmp = widget_button (w_buttons, value='Select All', uvalue='select_all')
...
xrealize,w_basemain,group=group,/screen,_extra=extra
;widget_control, w_basemain, /realize

widget_control, w_basemain, set_uvalue=state

xmanager, 'xsel_list_multi', w_basemain,/modal;, /no_block
