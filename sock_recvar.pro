...
;               8-Sep-2025, Zarro (Retired) - added support for writing to OUT_DIR
;
; Contact     : DZARRO@SOLAR.STANFORD.EDU
;-

pro sock_recvar,lun,name,value,err=err,verbose=verbose,format=format,session=session,$
           disconnected=disconnected,nowait=nowait,main_level=main_level,out_dir=out_dir,_extra=extra
 ...
 if 1 then mprint,'Receiving variable "'+name+'"'
 ...
 While (T ne bsize) Do Begin
  ...
  Endif
 Endwhile
 if 1 then mprint,'Read '+trim(t)+' bytes.'
 ...
 if format eq 'file' then begin
  if compressed then value=zlib_uncompress(temporary(buffer),type=1,dimensions=dimensions,/gzip) else value=temporary(buffer) 
