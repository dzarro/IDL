...
;               9-Jun-2026, Zarro (Retired)
;                - improved error checking
;                - removed SOHO roll correction (which should be handled externally)
;
...
pro index2map, index, data, maparr, noclobber=noclobber, outsize=outsize,$
               ...
               no_copy=no_copy,verbose=verbose,no_roll_correct=no_roll_correct,$
               no_angles=no_angles,wcs=wcs
...
;-- correct for SOHO roll
  ...
  ;  if verbose then mprint,'Correcting for SOHO 180 degree roll.',_extra=extra
...
;-- create map
  ...
  if is_string(err) then begin
   ...
   mprint,'skipping image '+num2str[i]+' -> '+smess,_extra=extra
   continue
  endif
