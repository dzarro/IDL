...
pro sock_content_http,response,type=type,size=bsize,date=date,$
               ...
               content_location=content_location,_ref_extra=extra,encoding=encoding

if is_blank(response) then begin   
 type='' & date='' & bsize=0l & disposition='' & location='' & code=0L
 chunked=0b & accept='' & content_location='' & encoding='' & resp=''
 return
endif
...
if arg_present(encoding) then encoding=key_value(resp,'Content-Encoding') else encoding=''

;-- get type

if arg_present(type) then type=key_value(resp,'Content-Type') else type=''

;-- get last modified date

if arg_present(date) then begin
 date=''
 temp=key_value(resp,'Last-Modified')
 if is_string(temp) then begin
  pie=str2arr(temp,delim=' ')
...
if arg_present(bsize) then begin
 bsize=0l
 dsize=key_value(resp,'Content-Length')
 if is_number(dsize) then bsize=long(dsize)
endif else bsize=0l
...
if arg_present(disposition) then begin
 disposition=''
 temp=key_value(resp,'Content-Disposition')
 if is_string(temp) then begin
  disp=stregex(temp,'.*filename="?([^"]*)"?', /extract, /subexp,/fold)
  file = disp[1]
