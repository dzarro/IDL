...
;               - fixed bug with missing HMI software in path
;
; Contact     : dzarro@solar.stanford.edu
;-
...
;-------------------------------------------------------------
pro hmi::prep,index,data,_ref_extra=extra,verbose=verbose,no_prep=no_prep,err=err
...
if ~no_prep && ~prepped && self->hmi::have_path(verbose=verbose,_extra=extra) then begin
 ...
 hmi_prep,tindex,data,oindex,odata,_extra=extra,/quiet,/use_ref,/nearest
