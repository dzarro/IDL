...
;               2-Aug-2025, Zarro (Retired)
;               - improved handling of returning inherited keyword values
;-
...
;-- check for modified input/output variables and return to scope of caller
 ...
 if ndata gt 0 then begin
  ...
  for i=1,ndata-1 do begin
   ...
   ;out_level=scope_level()+in_level-wait
   out_level=userdata[i].out_level
   if (var_name ne '') then begin
...
calls = SCOPE_TRACEBACK() & ncalls=n_elements(calls)
for i=1,n_params()-1 do begin
 ...
 for k=0,ncalls-1 do begin
  out_level=k+1
  var_input=scope_varname(scope_varfetch(var_name,/enter),level=out_level)
  if is_string(var_input[0]) then break
 endfor
 obridge->setvar,var_name,var_val
...
;  FOR k=N_ELEMENTS(calls)-1, 0L, -1 DO BEGIN
  FOR k=0,ncalls-1 DO BEGIN
   out_level=k+1
   var_input = SCOPE_VARNAME( SCOPE_VARFETCH(var_name, /REF_EXTRA), LEVEL=out_level)
   if is_string(var_input[0]) then break
  endfor
  
  obridge->setvar,var_name,var_val
  temp={var_name:var_name, var_input:var_input[0],in_level:out_level,out_level:out_level}
  userdata=[userdata,temp]
 endfor
endif
...
;-- ensure latest version of input procedure or function compiled

if is_string(funct) then tproc=funct else tproc=proc 
obridge->execute,'recompile,"'+tproc+'"'

if is_obj then obridge->setvar,obj_name,obj_val
dprint,'% CMD: ',cmd

wait=keyword_set(wait_thread)
obridge->execute,cmd,nowait=~wait
