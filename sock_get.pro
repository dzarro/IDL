...
if is_blank(file) then file=str_replace(stc.host,'.','_')
...
if pre_check then begin
 chk=sock_check(durl,chunked=chunked,disposition=disposition,size=bsize,$
               _extra=extra,date=rdate,err=err,$
...
if is_string(encoding) then begin
 if encoding eq 'gzip' then begin
  g_ofile=t_ofile+'.gz'
  file_copy,t_ofile,g_ofile,/overwrite,/allow_same,/noexpand_path,/force
  file_gunzip,g_ofile,verbose=verbose,/delete
 endif
endif

;-- update downloaded filename to original name
 
if is_string(disposition) && is_blank(out_name) then begin
...
err='' & status=0 & cancelled=0b & local_file=''
...
for i=0,np-1 do begin
 sock_get_main,url[i],out_name[i],local_file=lfile,err=err,status=lstatus,$
               cancelled=lcancelled,_extra=extra
 if is_string(err) || (lstatus eq 0) || lcancelled then continue
 local_file[i]=lfile
 status[i]=lstatus
 cancelled[i]=lcancelled
endfor
