...
;               DX, DY - pixel spacing
;               WCS - use WCS formalism if available
;
...
;               9 June 2026, Zarro (Retired) 
;               - added WCS
;
; Contact     : dzarro@solar.stanford.edu
;-

pro get_fits_cen,struct,xcen,ycen,dx=dx,dy=dy,time=time,err=err,verbose=verbose,wcs=wcs
...
if have_tag(struct,'pc1_1') && keyword_set(wcs) then begin
 if verbose then mprint,'Computing XCEN/YCEN from PC matrix.',_extra=extra
 comp_pc_cen,struct,xcen,ycen
 return
endif

if ~exist(dx) || ~exist(dy) then begin
...
if have_cen then zero_cen=(struct.xcen eq 0.) && (struct.ycen eq 0.)
...
if have_tag(struct,'crval1',/exact) and $
 ...
 if ~have_cen or zero_cen[0] then begin
  if verbose then mprint,'Computing XCEN/YCEN from CRVALs.',_extra=extra
  return
 endif
endif
...
if have_cen then begin
 xcen=struct.xcen
 ycen=struct.ycen
 if verbose then mprint,'Getting XCEN/YCEN from header fields.',_extra=extra
 return
endif
...
; lon=lon+tim2carr(dtime)
 ...
 if verbose then mprint,'Computing XCEN/YCEN from LAT/LON.',_extra=extra
endif
