...
;                13-June-2026, Zarro (Retired) - added MGET to retrieve array of properties
;
...
;----------------------------------------------------------------------------
;-- return multiple selected properties 

function map::mget,_extra=extra
count=self->get(/count)

for i=0,count-1 do begin
 prop=self->get(i,_extra=extra)
 if ~exist(prop) then return,prop
 temp=append_arr(temp,prop,/no_copy)
endfor
...
pro map::plot,k,_extra=extra,surface=surface,shade_surf=shade_surf,$
  ...
  error=0
  count=self->get(/count)
  if is_number(k) then ks = 0 > k[0] < (count-1)
  if (n_params() eq 0) then ks=0
   
  if (count gt 1) && (n_params() eq 0) then begin
   ids=self->mget(/id)
   times=self->mget(/time)
   ks=xsel_list_multi(ids+' '+times,cancel=cancel,/index,multiple=0)
  ...
  if ~self->has_data(ks,_extra=extra,/verbose) then return
  props=self->get(ks,/props)
  pmap=self->get(ks,/map,/pointer)
...
pro map::plotman, k, plotman_obj=plotman_obj, panel_description = panel_desc, $
  _ref_extra=extra,desc=desc,err=err
  ...
  for i=0,n_elements(val)-1 do begin
   ...
   plotman_obj -> new_panel, input=kobj, plot_type='image', desc =desc,$
      _extra=extra,nodup=0,replace=1,noclone=noclone
  endfor
