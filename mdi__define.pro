...
;               - fixed bug with missing MDI reader in path
;
...
for i=0,nfiles-1 do begin
 ...
 if have_path then rd_mdi,ofile[i],index,data else self->fits::read,ofile[i],index=index,data,_extra=extra
...
function mdi::search,tstart,tend,_ref_extra=extra,type=type,$
  times=times,sizes=sizes,count=count,verbose=verbose,continuum=continuum,err=err
